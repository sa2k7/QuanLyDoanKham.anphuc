...
- **⚠️ GOTCHA: Optimized Score** — - > 784 notes | Score threshold: >40
+ > 788 notes | Score threshold: 
- **⚠️ GOTCHA: Added JWT tokens authentication** — - - Strengthened types Comprehensive
+ - sys_01 in Users.vue — confirm
...
+       "id": "cdd93fa971dc7244",
+
- **sys_03 in QLDK.API.pdb** — File updated (external): QuanLyDoanKham.API/bin/Debug/net10.0/QLDK.API

## Project Standards

- sys_01 in shared-context.json — confirmed 3x
- Strengthened types Recent
- Optimized Score — hardens HTTP security headers — confirmed 3x
- sys_01 in Users.vue — confirmed 3x
- sys_01 in shared-context.json — confirmed 13x
- Fixed null crash in String — prevents null/undefined runtime crashes — confirmed 3x
- Optimized QLDK — confirmed 3x
- sys_09 in shared-context.json — confirmed 3x

## Known Fixes

- ❌ File updated (external): QuanLyDoanKham.API/build_errors_new.txt → ✅ problem-fix in build_log_errors.txt
- ❌ D:\QuanLyDoanKham\QuanLyDoanKham.API\Controllers\PayrollController.cs(241,21): error CS0117: 'Payrol → ✅ problem-fix in build_log.txt
- ❌ [Required(ErrorMessage = "Tên đăng nhập không được để trống.")] → ✅ problem-fix in AuthDtos.cs
- ❌ 📌 IDE AST Context: Modified symbols likely include [# Cáº£i thiá»‡n Há»‡ thĂ´ng ThĂ´ng bĂ¡o Lá»—i ( → ✅ problem-fix in task.md
- ❌ -         toast.error(e.response?.data || "Lỗi khi phân công. Nhân viên có thể bị trùng lịch!") → ✅ Fixed null crash in Staffs — prevents null/undefined runtime crashes

## Recent Decisions

- sys_04 in agent-rules.md
- sys_04 in main.css
- Optimized Score — hardens HTTP security headers
- sys_04 in Supplies.vue

## Learned Patterns

- Avoid: gotcha in QuanLyDoanKham.API.csproj.CoreCompileInputs.cache (seen 2x)
- Avoid: gotcha in QLDK.API.pdb (seen 4x)
- Decision: Optimized GlobalPropertiesHash (seen 2x)
