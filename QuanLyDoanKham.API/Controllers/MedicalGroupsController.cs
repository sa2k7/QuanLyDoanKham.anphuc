...
namespace QuanLyDoanKham.API.Controllers
{
    ...
    public class MedicalGroupsController : ControllerBase
    {
        ...
        public async Task<ActionResult<MedicalGroup>> PostMedicalGroup(MedicalGroupDto dto)
        {
            ...
            if (contract.Status == "Locked" || contract.Status == "Finished") 
                return BadRequest("Hợp đồng đã đóng hoặc kết thúc, không thể thêm đoàn khám.");
        ...
        public async Task<ActionResult<MedicalGroup>> AutoCreateFromContract(int contractId)
        {
            ...
            {
                GroupName = $"Đoàn khám {contract.Company.CompanyName} - Lần {existingGroupsCount + 1}",
        ...
        public async Task<IActionResult> ExportGroupsExcel()
        {
            ...
            using (var workbook = new ClosedXML.Excel.XLWorkbook())
            {
                ...
                for (int i = 0; i < groups.Count; i++)
                {
                    ...
                    worksheet.Cell(i + 2, 2).Value = groups[i].HealthContract.Company.CompanyName;
