<template>
  <div class="premium-card p-4 mb-6 overflow-hidden border-indigo-100 bg-indigo-50/10 shadow-sm transition-all duration-300">
    ...
    <!-- Mode: Manual -->
    <form v-if="createMode === 'manual'" @submit.prevent="handleManualSubmit" class="grid grid-cols-1 md:grid-cols-6 gap-x-4 gap-y-4">
        <!-- Row 1: 2 + 3 + 1 = 6 -->
        ...
        <!-- Row 2: 1 + 1 + 3 + 1 = 6 -->
        <div class="md:col-span-1 flex flex-col gap-1.5">
            <label class="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-1">Bắt đầu khám</label>
        ...
        <div class="md:col-span-1 flex flex-col justify-end">
            <button type="submit" :disabled="isLoading" class="h-11 w-full rounded-xl bg-slate-800 text-white font-black text-[9px] uppercase tracking-widest hover:bg-slate-900 transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2">
