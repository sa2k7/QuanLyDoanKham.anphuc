  ...
  <div class="space-y-6 animate-fade-in pb-20">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
      <div>
        ...
        <p class="text-slate-400 font-bold uppercase tracking-widest text-[9px] mt-2">Nội bộ: Tổng hợp lương cố định & Thù lao đi đoàn</p>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="premium-card p-6 bg-white border border-slate-50">
            ...
            <p class="text-2xl font-black text-slate-800 tracking-tighter">{{ formatPrice(totalPayroll) }}</p>
        </div>
        <div class="premium-card p-6 bg-white border border-slate-50">
            <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Số lượng nhân sự hưởng lương</p>
            <p class="text-2xl font-black text-slate-800 tracking-tighter">{{ payrollList.length }} <span class="text-xs text-slate-400">Thành viên</span></p>
        </div>
        <div class="premium-card p-6 bg-slate-900 text-white">
            <button @click="exportExcel" class="w-full h-full flex items-center justify-center gap-3 font-black text-xs uppercase tracking-widest">
                <Download class="w-5 h-5" /> XUẤT FILE EXCEL
            </button>
        </div>
    </div>
    ...
    <div class="premium-card bg-white border border-slate-100 overflow-hidden">
        <div class="p-6 border-b border-slate-50 flex items-center gap-4 bg-slate-50/30">
            <div class="relative flex-1">
                ...
                <input v-model="searchQuery" placeholder="Tìm tên nhân viên..." class="w-full pl-10 pr-4 py-2 rounded-xl bg-white border border-slate-200 text-xs font-bold outline-none" />
            </div>
        </div>
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-slate-50">
                    <tr v-for="item in filteredList" :key="item.staffId" class="text-xs hover:bg-slate-50/50 transition-all">
                        ...
                        <td class="p-4 font-bold text-slate-500">{{ item.jobTitle }}</td>
                        <td class="p-4 text-right font-bold">{{ formatPrice(item.baseSalary) }}</td>
                        ...
                        <td class="p-4 text-center">
                            <button @click="showDetails(item)" class="text-indigo-600 hover:underline font-black uppercase text-[10px]">Cơ cấu</button>
                        </td>
                    </tr>
                </tbody>
            </table>
    ...
    <div v-if="detailItem" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
        <div class="bg-white w-full max-w-2xl rounded-[2rem] shadow-2xl overflow-hidden flex flex-col animate-fade-in-up">
            <div class="p-8 border-b border-slate-100 flex justify-between items-center">
                <h3 class="text-xl font-black text-slate-800 uppercase">Cơ cấu lương: {{ detailItem.fullName }}</h3>
            ...
            <div class="p-8 space-y-6 overflow-y-auto max-h-[60vh]">
                <div class="bg-slate-50 p-6 rounded-2xl grid grid-cols-2 gap-4">
                    <div>
                        <p class="text-[9px] font-black text-slate-400 uppercase">Lương cố định</p>
                        <p class="text-lg font-black text-slate-700">{{ formatPrice(detailItem.baseSalary) }}</p>
                    </div>
                    <div>
                        <p class="text-[9px] font-black text-slate-400 uppercase">Số ngày đi đoàn</p>
                        <p class="text-lg font-black text-indigo-600">{{ detailItem.totalDays }} Ngày</p>
                    </div>
                </div>

                <div class="space-y-4">
                    <p class="text-[10px] font-black text-slate-400 uppercase">Chi tiết các đoàn đã tham gia</p>
                    <div class="space-y-2">
                        <div v-for="d in detailItem.details" :key="d.groupId" class="flex justify-between items-center p-4 bg-white border border-slate-50 rounded-xl">
                            <div>
                                ...
                                <p class="text-[9px] font-bold text-slate-400 uppercase">{{ formatDate(d.examDate) }} • Hệ số: {{ d.shiftType }}</p>
                            </div>
            ...
            <div class="p-8 bg-slate-900 text-white flex justify-between items-center">
                <div>
                    <p class="text-[9px] font-black text-slate-500 uppercase">Tổng cộng Thực lãnh</p>
                    <p class="text-3xl font-black text-emerald-400 tracking-tighter">{{ formatPrice(detailItem.totalSalary) }}</p>
                </div>
                <button @click="detailItem = null" class="px-8 py-3 bg-slate-800 text-white rounded-xl font-black text-xs uppercase">ĐÓNG</button>
            </div>
        </div>
    </div>
  </div>
